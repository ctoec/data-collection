...
export function weightedUndefinableBoolean(
  probabilityTruePercentage: number,
  probabilityNotCollectedPercentage?: number
) {
  if (
    probabilityNotCollectedPercentage &&
    weightedBoolean(probabilityNotCollectedPercentage)
  ) {
    return UndefinableBoolean.NotCollected;
  }
  return weightedBoolean(probabilityTruePercentage)
