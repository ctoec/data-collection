...
export class InitialMigration1600383973776 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "funding_space" ("id" int NOT NULL IDENTITY(1,1), "capacity" int NOT NULL, "source" nvarchar(255)CHECK( source IN ('CDC','CSR','PSR','SHS','SS') ) NOT NULL, "ageGroup" nvarchar(255) CHECK( ageGroup IN ('InfantToddler','Preschool','SchoolAge') ) NOT NULL, "time" nvarchar(255) CHECK( time IN ('FullTime','PartTime','SplitTime','FullDay','PartDay','ExtendedDay','ExtendedYear','School') ) NOT NULL, "organizationId" int NOT NULL, CONSTRAINT "UQ_Source_AgeGroup_Time_Organization" UNIQUE ("source", "ageGroup", "time", "organizationId"), CONSTRAINT "PK_0f92a2c9df81f7cecc05d28c99d" PRIMARY KEY ("id"))`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "enrollment" ("id" int NOT NULL IDENTITY(1,1), "childId" uniqueidentifier NOT NULL, "siteId" int NOT NULL, "ageGroup" nvarchar(255) CHECK( ageGroup IN  ('InfantToddler','Preschool','SchoolAge') ), "entry" date, "exit" date, "exitReason" nvarchar(255), "authorId" int, "updatedAt" datetime2 NOT NULL CONSTRAINT "DF_3a787207f5857d5ebc444c2169c" DEFAULT getdate(), CONSTRAINT "PK_7e200c699fa93865cdcdd025885" PRIMARY KEY ("id"))`
    );
    await queryRunner.query(
