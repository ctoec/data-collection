/* tslint:disable */
/* eslint-disable */
/**
 * data-collection
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 0.0.1
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import {
  CdcReport,
  CdcReportFromJSON,
  CdcReportFromJSONTyped,
  CdcReportToJSON,
  FundingSpace,
  FundingSpaceFromJSON,
  FundingSpaceFromJSONTyped,
  FundingSpaceToJSON,
  ReportingPeriod,
  ReportingPeriodFromJSON,
  ReportingPeriodFromJSONTyped,
  ReportingPeriodToJSON,
} from './';

/**
 *
 * @export
 * @interface FundingTimeSplitUtilization
 */
export interface FundingTimeSplitUtilization {
  /**
   *
   * @type {number}
   * @memberof FundingTimeSplitUtilization
   */
  id: number;
  /**
   *
   * @type {number}
   * @memberof FundingTimeSplitUtilization
   */
  reportingPeriodId: number;
  /**
   *
   * @type {ReportingPeriod}
   * @memberof FundingTimeSplitUtilization
   */
  reportingPeriod?: ReportingPeriod;
  /**
   *
   * @type {number}
   * @memberof FundingTimeSplitUtilization
   */
  reportId: number;
  /**
   *
   * @type {CdcReport}
   * @memberof FundingTimeSplitUtilization
   */
  report?: CdcReport;
  /**
   *
   * @type {number}
   * @memberof FundingTimeSplitUtilization
   */
  fundingSpaceId: number;
  /**
   *
   * @type {FundingSpace}
   * @memberof FundingTimeSplitUtilization
   */
  fundingSpace?: FundingSpace;
  /**
   *
   * @type {number}
   * @memberof FundingTimeSplitUtilization
   */
  fullTimeWeeksUsed: number;
  /**
   *
   * @type {number}
   * @memberof FundingTimeSplitUtilization
   */
  partTimeWeeksUsed: number;
}

export function FundingTimeSplitUtilizationFromJSON(
  json: any
): FundingTimeSplitUtilization {
  return FundingTimeSplitUtilizationFromJSONTyped(json, false);
}

export function FundingTimeSplitUtilizationFromJSONTyped(
  json: any,
  ignoreDiscriminator: boolean
): FundingTimeSplitUtilization {
  if (json === undefined || json === null) {
    return json;
  }
  return {
    id: json['id'],
    reportingPeriodId: json['reportingPeriodId'],
    reportingPeriod: !exists(json, 'reportingPeriod')
      ? undefined
      : ReportingPeriodFromJSON(json['reportingPeriod']),
    reportId: json['reportId'],
    report: !exists(json, 'report')
      ? undefined
      : CdcReportFromJSON(json['report']),
    fundingSpaceId: json['fundingSpaceId'],
    fundingSpace: !exists(json, 'fundingSpace')
      ? undefined
      : FundingSpaceFromJSON(json['fundingSpace']),
    fullTimeWeeksUsed: json['fullTimeWeeksUsed'],
    partTimeWeeksUsed: json['partTimeWeeksUsed'],
  };
}

export function FundingTimeSplitUtilizationToJSON(
  value?: FundingTimeSplitUtilization | null
): any {
  if (value === undefined) {
    return undefined;
  }
  if (value === null) {
    return null;
  }
  return {
    id: value.id,
    reportingPeriodId: value.reportingPeriodId,
    reportingPeriod: ReportingPeriodToJSON(value.reportingPeriod),
    reportId: value.reportId,
    report: CdcReportToJSON(value.report),
    fundingSpaceId: value.fundingSpaceId,
    fundingSpace: FundingSpaceToJSON(value.fundingSpace),
    fullTimeWeeksUsed: value.fullTimeWeeksUsed,
    partTimeWeeksUsed: value.partTimeWeeksUsed,
  };
}
